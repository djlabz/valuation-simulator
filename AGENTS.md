...
| **Arquivo bruto de procedência fora do versionamento, e a decisão é herdada.** O `.srt`, o texto com timestamp e o áudio que sustentam a procedência de qualquer afirmação extraída de vídeo moram em `ingest/`, fora do git por D-042. Consequência: **o material que permite conferir um timestamp meses depois não sobrevive a uma troca de máquina**, que é o mesmo efeito do arquivo perdido da D-080. A decisão de não versionar pode continuar valendo, mas hoje ela é **herdada de uma regra escrita para outro propósito**, quando `ingest/` era só depósito de PDF de terceiro, e merece ser tomada de propósito. Não foi decidida (D-089 a D-092) | Antes da Etapa do Conhecimento, que é quando o volume de material bruto cresce |
